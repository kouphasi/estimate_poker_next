...
### 目標
認証なしで「ログイン」してセッションを管理できる機能（ニックネーム保存程度）
...
2. **GET /api/users/[userId]/sessions** - セッション一覧取得API
   - ユーザーが作成したセッション一覧を取得
   - 作成日時の降順でソート

3. **DELETE /api/sessions/[shareToken]** - セッション削除API
   - オーナー権限チェック実装
