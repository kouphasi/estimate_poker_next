...
describe('GetProjectUseCase', () => {
  ...
  beforeEach(() => {
    ...
    useCase = new GetProjectUseCase(mockProjectRepository, mockSessionRepository);
  });
  ...
  it('should include sessions in project details', async () => {
    const project = Project.create(
      'project-123',
      'Test Project',
      null,
    ...
    const sessions = [
      ...
      EstimationSession.create(
        'session-2',
        null,
        ShareToken.fromString('ABCDabcd12340002'),
        OwnerToken.fromString('ABCDabcd1234567890123456789012cd'),
        'owner-123',
        'project-123'
      ),
    ];
    ...
    expect(result.sessions[1].name).toBeNull();
  });
